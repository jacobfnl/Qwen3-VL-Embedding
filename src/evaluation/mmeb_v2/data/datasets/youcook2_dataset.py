...
def data_prepare(batch_dict, **kwargs):
    ...
    for video_name, video_path, text in zip(
        batch_dict['id'], 
        batch_dict["video_path"], 
        batch_dict['sentence']
    ):
        query_inputs.append({
            "text": text,
        ...
        actual_video_path = os.path.join(video_root, os.path.basename(video_path))
        frame_dir = os.path.join(frame_root, video_name)
        if not os.path.exists(frame_dir):
            save_frames(video_path=actual_video_path, frame_dir=frame_dir, max_frames_saved=max_frames_saved)
